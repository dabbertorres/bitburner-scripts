...
//const exec_servers = ["home"].concat(Array(15).fill().map((_, i) => "server-" + i));
...
export async function main(ns) {
	...
	const exec_servers = ["home"].concat(ns.getPurchasedServers());

	await find_not_hacked_optimally(ns, exec_servers, data.script, data.start);
}

/**
* @param {NS} ns
**/
export async function find_not_hacked_optimally(ns, runners, script, ...target_chain) {
...
export async function spread(ns, script, target_chain, neighbors) {
	for (let neighbor of neighbors) {
		await find_not_hacked_optimally(ns, script, ...target_chain, neighbor);
	}
}
...
export function get_neighbors(ns, ...target_chain) {
	...
	return ns.scan(target).filter(neighbor => !exec_servers.includes(neighbor) && neighbor !== parent);
}